...
public class LauncherGui extends JFrame {
	...
	private PanelAlert panelAlert=new PanelAlert(new JLabel(new ImageIcon(getClass().getResource("alert.png"))));
	private PanelAlert panelNoAlert=new PanelAlert(new JLabel(new ImageIcon(getClass().getResource("notAlert.png"))));
	private DrawGraph barChart=new DrawGraph();
	...
	class InitialPanel extends JPanel{
		...
		private GridBagConstraints gbcST=new GridBagConstraints();
		...
		private	JLabel analisiLogo=new JLabel(new ImageIcon(getClass().getResource("lineGraph.png")));
		...
		//	hashtag.setBorder(new LineBorder(Color.WHITE, 4, true));
			...
			int asciiNum=22;
			JLabel iterazioneLabel=new JLabel(Character.toString((char)asciiNum)+" Status Iterazione");
			iterazioneLabel.setFont(new Font("Arial", Font.BOLD,18));
			iterazioneLabel.setForeground(Color.ORANGE);
			JLabel alertLabel=new JLabel(Character.toString((char)asciiNum)+" Status Alert");
			alertLabel.setFont(new Font("Arial", Font.BOLD,18));
			alertLabel.setForeground(new Color(0,255,124));
			JLabel noAlertLabel=new JLabel(Character.toString((char)asciiNum)+" Status Not Alert");
			noAlertLabel.setFont(new Font("Arial", Font.BOLD,18));
			noAlertLabel.setForeground(new Color(255,22,22));
			gbcST.fill = GridBagConstraints.HORIZONTAL;
	        gbcST.gridx = 0;
	        gbcST.gridy = 0;
			tmp.add(analisiIterazione, gbcST);
	        gbcST.gridx = 0;
	        gbcST.gridy = 2;
			tmp.add(iterazioneLabel, gbcST);
	        gbcST.gridx = 0;
	        gbcST.gridy = 3;
			tmp.add(alertLabel, gbcST);
	        gbcST.gridx = 0;
	        gbcST.gridy = 4;
			tmp.add(noAlertLabel, gbcST);
	        gbcST.gridx = 0;
	        gbcST.gridy = 5;
			tmp.add(analisiLogo, gbcST);
			...
			tab.addTab("TWEET SCARTATI",panelNoAlert);
			tab.addTab("RICERCA EMERGENZA", panelAnalisiTotale);
					...
					public void mouseClicked(MouseEvent e) {
						if(search.isEnabled()) {
							if(listModel.size()==0) {
								...
								controller.normalSearch(searching, listModel, pathFile, panelTable,panelAlert,panelNoAlert, barChart, saving, search);
							}
						}
					}});
					...
					public void mouseClicked(MouseEvent e) {
						if(searchAvanzata.isEnabled()) {
							if(listModelAvanzata.size()==0) {
								...
								check.setEnabled(false);
								controller.advanceSearch(searching, listModelAvanzata, latitudine, longitudine, area, giorno, pathFileAvanzata, panelTable,panelAlert,panelNoAlert, barChart, saving, searchAvanzata);
							}
						}
					}});
		}//listener
